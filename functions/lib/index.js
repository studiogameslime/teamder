...
async function runFindFillerCandidates() {
    ...
    for (const doc of snap.docs) {
        ...
        const minTrust = typeof game.fillerMinTrust === 'number' ? game.fillerMinTrust : 70;
        const newlyPushed = {};
        ...
        if (pushesThisGame > 0) {
            ...
            if (now - lastFallbackAt >= FILLER_NO_CANDIDATES_COOLDOWN_MS &&
                game.createdBy) {
                await createNotificationOnce({
                ...
                await doc.ref.set({ fillerNoCandidatesAt: now, updatedAt: now }, { merge: true });
                fallbackPushed += 1;
            }
        }
    }
