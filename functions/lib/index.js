 ...
 */
function pruneUidsFromTeamsSrv(d, gone) {
    if (gone.size === 0)
        return {};
    const out = {};
    const draft = d.draftTeams;
    if (draft?.teams) {
        const inTeams = draft.teams.some((t) => (t.playerIds ?? []).some((p) => gone.has(p)));
        const inLeftHome = (draft.leftHome ?? []).some((l) => l.playerId && gone.has(l.playerId));
        if (inTeams || inLeftHome) {
            out.draftTeams = {
                ...
                ...(inLeftHome
                    ? {
                        leftHome: (draft.leftHome ?? []).filter((l) => !(l.playerId && gone.has(l.playerId))),
                    }
                    : {}),
            };
        }
    ...
    if (rotation) {
        const inLoans = (rotation.loans ?? []).some((l) => l.playerId && gone.has(l.playerId));
        const inBase = (rotation.baseTeams ?? []).some((t) => (t.playerIds ?? []).some((p) => gone.has(p)));
        if (inLoans || inBase) {
            out.rotation = {
                ...rotation,
                ...(inLoans
                    ? {
                        loans: (rotation.loans ?? []).filter((l) => !(l.playerId && gone.has(l.playerId))),
                    }
                    : {}),
                ...(inBase
                    ? {
...
async function loadUsers(uids) {
    ...
    for (let start = 0; start < unique.length; start += CHUNK) {
        ...
        const privateRefs = slice.map((u) => db.collection('users').doc(u).collection('private').doc('push'));
...
const FANOUT_NOTIF_TYPES = new Set([
    'newGameInCommunity',
...
async function isFanoutSenderAuthorized(notif) {
...
async function resolveRecipients(notif) {
    ...
    if (notif.type === 'gameReminder' ||
        ...
        if (notif.type === 'gameCanceledOrUpdated' &&
            ...
            payload.directedTo) {
            return loadUsers([payload.directedTo]);
        }
...
async function deliverBatch(type, recipients, message, data) {
    ...
    const prefKey = type === 'approved' || type === 'rejected'
        ? 'approvedRejected'
...
exports.onNotificationCreated = (0, firestore_1.onDocumentCreated)('notifications/{id}', async (event) => {
    ...
    if (notif.type === 'gameCanceledOrUpdated' &&
        ...
        const latchCategory = notif.payload?.action === 'updated' ? 'update' : 'cancel';
        const latchRef = db
...
async function runSendRsvpNudges() {
    ...
    for (const doc of snap.docs) {
        ...
        const guestsCount = Array.isArray(g.guests)
            ? g.guests.filter((x) => !x?.waitlisted)
                .length
            : 0;
 ...
 * live-state to strip.
 */
async function runCreateSeriesOccurrences() {
    const now = Date.now();
    const snap = await db
    ...
    for (const doc of snap.docs) {
        ...
        // played.
        if (now < last + RECURRING_CLONE_DELAY_MS)
            continue;
...
async function runCloneRecurringGames() {
    ...
    for (const doc of snap.docs) {
        ...
        if (g.seriesId)
            continue;
        if (g.groupId && seriesGroups.has(g.groupId))
            continue;
    ...
    for (const gameDoc of snap.docs) {
        ...
        try {
            await db.runTransaction(async (tx) => {
                ...
                const gameTtlMs = typeof d.waitlistApprovalTimeoutMinutes === 'number' &&
...
exports.onGameRosterChanged = (0, firestore_1.onDocumentWritten)('games/{gameId}', async (event) => {
        ...
        if (before && delStatus !== 'finished' && delStatus !== 'cancelled') {
            ...
            if (roster.length > 0) {
                try {
                    ...
                    console.error('[onGameRosterChanged] delete cancellation push failed', event.params.gameId, err);
                }
            }
        }
        return;
    ...
    const pruneOk = afterStatus === undefined ||
        afterStatus === 'open' ||
    ...
    const promoteOk = afterStatus === undefined ||
        afterStatus === 'open' ||
        afterStatus === 'scheduled' ||
        afterStatus === 'locked';
    if (rosterShrank && pruneOk) {
        ...
        const departed = new Set(beforePlayers.filter((p) => !stillHere.has(p)));
        ...
        try {
            await db.runTransaction(async (tx) => {
                ...
                if (promoteOk &&
                    ...
                    if (d.waitlistApprovalRequired === false) {
                        ...
                        updates.participantIds = Array.from(new Set([...players, ...waitlist, ...pending]));
                    }
                    else {
            ...
            console.error('[onGameRosterChanged] promote/prune failed', event.params.gameId, err);
        }
        if (promotedUid) {
            try {
                await createNotificationOnce({
                ...
                console.error('[onGameRosterChanged] spotOpened push failed', event.params.gameId, err);
            }
        }
    }
    ...
    if (before?.status !== 'finished' &&
        ...
        if (creditedNow) {
            try {
                ...
                const goalsFor10 = Math.max(BENCH_FLOOR, newGCnt > 0 ? newGSum / newGCnt : DEFAULT_GOALS_FOR_10);
                const assistsFor10 = Math.max(BENCH_FLOOR, newACnt > 0 ? newASum / newACnt : DEFAULT_ASSISTS_FOR_10);
                const pts = (g, a) => g * 2 + a;
                ...
                standingBatch.set(csRef, {
                    kingGoalsSum: admin.firestore.FieldValue.increment(sessionKingGoals * addG),
                    kingGoalsCount: admin.firestore.FieldValue.increment(addG),
                    kingAssistsSum: admin.firestore.FieldValue.increment(sessionKingAssists * addA),
    ...
    const activeG = (gs) => Array.isArray(gs)
        ? gs.filter((x) => !x?.waitlisted).length
        : 0;
    const beforeCount = (before?.players?.length ?? 0) + activeG(before?.guests);
...
'ratings/{ratedUserId}/votes/{raterUserId}', async (event) => {
    ...
    await applyVoteDelta(db.collection('ratings').doc(ratedUserId), ratedUserId, countDelta, sumDelta, event.id);
});
...
exports.onVoteWrittenLegacy = (0, firestore_1.onDocumentWritten)('groups/{groupId}/ratings/{ratedUserId}/votes/{raterUserId}', async (event) => {
    ...
    await applyVoteDelta(db
        ...
        .doc(groupId)
        .collection('ratings')
        .doc(ratedUserId), ratedUserId, countDelta, sumDelta, event.id);
});
// Shared transactional count/sum/average updater for a rating summary doc.
async function applyVoteDelta(summaryRef, ratedUserId, countDelta, sumDelta, eventId) {
    await db.runTransaction(async (tx) => {
        const snap = await tx.get(summaryRef);
        const data = snap.exists && snap.data()
            ? snap.data()
            : { count: 0, sum: 0, appliedEvents: [] };
...
exports.setGuestRating = (0, https_1.onCall)({ enforceAppCheck: ENFORCE_APP_CHECK }, async (request) => {
    ...
    if (data.rating === null || data.rating === undefined || data.rating === 0) {
        ...
        throw new https_1.HttpsError('invalid-argument', 'rating must be within (0,5] or null');
    }
    const ref = db.collection('games').doc(gameId);
    await db.runTransaction(async (tx) => {
        const snap = await tx.get(ref);
...
// cleaned here.
exports.deleteMyAccount = (0, https_1.onCall)({ enforceAppCheck: ENFORCE_APP_CHECK }, async (request) => {
    const auth = request.auth;
    if (!auth?.uid) {
        throw new https_1.HttpsError('unauthenticated', 'sign-in required');
    }
        ...
        for (const field of ['adminIds', 'playerIds', 'pendingPlayerIds']) {
            ...
            for (const gd of snap.docs) {
                ...
                if (isCreator && admins.length === 0 && players.length === 0) {
                    ...
                    b.delete(db.collection('groupsPublic').doc(gd.id));
                    await bump();
                    b.delete(db.collection('communityShowcase').doc(gd.id));
                    await bump();
                    b.delete(gd.ref);
                    await b.commit();
                }
    ...
    try {
        ...
        for (let i = 0; i < MAX_BATCHES; i++) {
            ...
            if (msgs.empty)
                break;
            const b = db.batch();
            msgs.docs.forEach((m) => b.update(m.ref, {
    ...
    try {
        ...
        throw new https_1.HttpsError('internal', 'account data cleared but auth delete failed');
    }
    return { ok: true };
});
...
exports.reportChatMessage = (0, https_1.onCall)({ enforceAppCheck: ENFORCE_APP_CHECK }, async (request) => {
    const auth = request.auth;
    if (!auth?.uid) {
        throw new https_1.HttpsError('unauthenticated', 'sign-in required to report');
    }
    const data = (request.data ?? {});
    ...
    if (!parentId ||
        ...
        throw new https_1.HttpsError('invalid-argument', 'invalid report target');
    }
    const collByScope = {
        game: 'games',
        community: 'groups',
...
exports.notifyPlayerCancelled = (0, https_1.onCall)({ enforceAppCheck: ENFORCE_APP_CHECK }, async (request) => {
    ...
    const related = (game.players ?? []).includes(callerUid) ||
...
exports.promoteOrphanToGroup = (0, https_1.onCall)({ enforceAppCheck: ENFORCE_APP_CHECK }, async (request) => {
    ...
    if (effectiveFromGameId) {
        try {
            ...
            const tiedRounds = roundsSnap.docs.filter((d) => d.data().winnerSide === 'tie').length;
...
exports.createGroupCallable = (0, https_1.onCall)({ enforceAppCheck: ENFORCE_APP_CHECK }, async (request) => {
    ...
    const geo = input;
    const validCoord = (v) => typeof v === 'number' && Number.isFinite(v) ? v : undefined;
    ...
    if (coverImageId !== undefined)
        groupDoc.coverImageId = coverImageId;
    const publicDoc = {
        id: groupId,
        name,
        normalizedName: normaliseGroupName(name),
    ...
    if (coverImageId !== undefined)
        publicDoc.coverImageId = coverImageId;
...
exports.updateShowcaseOnGameChange = (0, firestore_1.onDocumentWritten)('games/{gameId}', async (event) => {
    ...
    if (beforeTerminal && afterTerminal) {
        ...
        const changed = showcaseKeys.some((k) => JSON.stringify(b?.[k]) !== JSON.stringify(a?.[k]));
        if (!changed)
            return;
    }
    try {
...
exports.inviteFriendsToGroup = (0, https_1.onCall)({ enforceAppCheck: ENFORCE_APP_CHECK }, async (request) => {
    ...
    const maxMembers = typeof g.maxMembers === 'number' && g.maxMembers > 0
        ? Math.min(g.maxMembers, 500)
        : 500;
    const currentCount = (g.playerIds?.length || 0) + (g.pendingPlayerIds?.length || 0);
    ...
    await groupRef.set({
        [g.isOpen ? 'playerIds' : 'pendingPlayerIds']: admin.firestore.FieldValue.arrayUnion(...accepted),
        updatedAt: Date.now(),
    }, { merge: true });
...
exports.commitRoundStats = (0, https_1.onCall)({ enforceAppCheck: ENFORCE_APP_CHECK }, async (request) => {
    ...
    for (const uid of new Set([...A, ...B])) {
        ...
        batch.set(db.collection('gamePlayerStats').doc(`${gameId}__${uid}`), {
            gameId,
            userId: uid,
            rounds: inc(1),
...
exports.saveGamePhysical = (0, https_1.onCall)({ enforceAppCheck: ENFORCE_APP_CHECK }, async (request) => {
    const uid = request.auth?.uid;
    if (!uid)
        throw new https_1.HttpsError('unauthenticated', 'sign in required');
    const { gameId, metrics } = (request.data ?? {});
    if (!gameId || !metrics || typeof metrics !== 'object') {
        throw new https_1.HttpsError('invalid-argument', 'gameId + metrics required');
    }
    const snap = await db.collection('games').doc(gameId).get();
    if (!snap.exists)
        throw new https_1.HttpsError('not-found', 'game not found');
...
exports.savePitchCalibration = (0, https_1.onCall)({ enforceAppCheck: ENFORCE_APP_CHECK }, async (request) => {
    const uid = request.auth?.uid;
    if (!uid)
        throw new https_1.HttpsError('unauthenticated', 'sign in required');
    const { groupId, corners } = (request.data ?? {});
    if (!groupId || !Array.isArray(corners) || corners.length !== 4) {
        throw new https_1.HttpsError('invalid-argument', 'groupId + 4 corners required');
    }
    const clean = corners.map((c) => {
        ...
        if (Number.isNaN(lat) || Number.isNaN(lng) ||
            ...
            throw new https_1.HttpsError('invalid-argument', 'invalid corner coords');
        }
        return { lat, lng };
    });
    const grp = await db.collection('groups').doc(groupId).get();
    ...
    const isAdmin = grp.data()?.creatorId === uid ||
        (grp.data()?.adminIds ?? []).includes(uid);
    if (!isAdmin)
        throw new https_1.HttpsError('permission-denied', 'admin only');
    await db.collection('groups').doc(groupId).set({ pitchCalibration: { corners: clean, by: uid, at: Date.now() } }, { merge: true });
    return { ok: true };
});
// ── Retro goals ─────────────────────────────────────────────────────────────
...
exports.onFeedbackCreated = (0, firestore_1.onDocumentCreated)('feedback/{feedbackId}', async (event) => {
    ...
    if ((await ref.get()).exists)
        return;
    const category = d.category === 'ui' || d.category === 'feature' ? d.category : 'bug';
