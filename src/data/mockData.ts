...
export const mockGamesV2: Game[] = [
  ...
  // ── QA fixtures for the 2 new roster features (ME = organizer/admin) ──
  // A) SCHEDULED game (registration not open yet) → MatchDetails shows the
  //    "שריין מקומות" entry. Empty roster so reserving is visible.
  {
    id: 'gv2-reserve-test',
    groupId: 'g1',
    title: 'בדיקה · שריון מקומות',
    startsAt: inDays(6, 20, 0),
    registrationOpensAt: inDays(3, 20, 0),
    fieldName: 'המגרש הקבוע',
    fieldLat: 32.0853,
    fieldLng: 34.7818,
    maxPlayers: 12,
  ...
  // B) OPEN game with a free spot (9/10) + a 4-person waitlist → MatchPlayers
  //    shows the ↑/↓ reorder + "להרכב" controls; move-to-roster succeeds.
  {
    id: 'gv2-reorder-test',
    groupId: 'g1',
    title: 'בדיקה · סידור המתנה',
    startsAt: inDays(2, 20, 0),
    fieldName: 'המגרש הקבוע',
    fieldLat: 32.0853,
    fieldLng: 34.7818,
    maxPlayers: 10,
    minPlayers: 8,
    players: mockPlayers.slice(0, 9).map((p) => p.id),
    waitlist: mockPlayers.slice(9, 13).map((p) => p.id),
    pending: [],
    participantIds: unionIds(
      mockPlayers.slice(0, 9).map((p) => p.id),
      mockPlayers.slice(9, 13).map((p) => p.id),
      [],
    ),
    status: 'open',
    locked: false,
